...
public class ManejadorArchivo 
{
      ...
      public void GrabarArchivo(RandomAccessFile archivo,byte[] Nombre,byte[] ApellidoP,
            byte[] ApellidoM,byte[] edad,byte[] nombreImagen,long posicion,int opcion)
    {
        ...
        {
        try {
            while(archivo.getFilePointer()<archivo.length()&&band!=1)
            {
    ...
    public long Cambio(RandomAccessFile archivo,byte[] Nombre,byte[] ApellidoP,byte[] ApellidoM,int op)
    {
    ...
    public void MenuCambio(Long inicio,RandomAccessFile archivo)
{
    ...
    String salto="\r\n";
    for(int i=borrado.length();i<200;i++)
    {
    borrado+=" ";
    }
        try {
